...
public class FetcherBolt implements IRichBolt {
	...
	public void execute(Tuple input) {
		...
		try {
			...
			for (Object obj : feed.getEntries()) {
				...
				collector.emit(new Values(syndEntry.getLink(), entryDate.getTime(),
						bRemoval.removeBoilerplate(syndEntry.getDescription().getValue())));
			}
			collector.ack(input);
		} catch (Throwable t) {
			t.printStackTrace();
	...
	private Date getDate(SyndEntry syndEntry, SyndFeed feed) {
		return syndEntry.getUpdatedDate() == null
				? (syndEntry.getPublishedDate() == null ? feed.getPublishedDate() : syndEntry.getPublishedDate())

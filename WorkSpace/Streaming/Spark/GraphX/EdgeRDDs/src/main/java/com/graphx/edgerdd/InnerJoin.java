...
public class InnerJoin implements Serializable {
	...
	static Logger log = Logger.getLogger(InnerJoin.class);
	...
	public void test() {
		...
		EdgeRDD mapVertex = graph.edges().innerJoin(mapRdd, new JoinFunction(), ClassTag$.MODULE$.apply(Integer.class),
				ClassTag$.MODULE$.apply(Integer.class));

		mapVertex.toJavaRDD().foreach(f -> {
			log.info(f);
		});

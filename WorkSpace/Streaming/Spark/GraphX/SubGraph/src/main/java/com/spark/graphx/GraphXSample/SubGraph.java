...
public class SubGraph implements Serializable {
	...
	Logger log = Logger.getLogger(SubGraph.class);
	...
	public void test() {
		...
		Graph<String[], String> graph1 = graph.subgraph(new EdgeFunction(), new VertexFunction());

		List<Tuple2<Object, String[]>> list3 = graph1.<String>vertices().toJavaRDD().collect();
		...
		log.info("No Of Vertices : " + graph1.ops().numVertices());
		log.info("No Of Edges : " + graph1.ops().numEdges());

	}

	public static void main(String[] args) {
		SubGraph pr = new SubGraph();
		pr.test();
	}
